...
Var
  ...
  vltotpis: Double;
  vltotcofins: Double;
  vlNFCalculado: Double; // [#2] reconciliação do total da NF-e antes de assinar
  vlErrosFiscais: TStringList; // [#4] pendências de NCM/CFOP por item
...
Begin
  ...
  *)
  ...
  // [#9] Proteção: não gerar nova chave/número quando já existe uma chave vinculada no banco.
  // Não depende mais apenas do arquivo XML em disco — evita duplicidade fiscal caso o XML
  // tenha sido movido ou apagado mas a chave continue registrada em mes.meschavenfe.
  if Trim(mesmeschavenfe.AsString) <> '' then
  begin
    if (Trim(mesmesprotocolo.AsString) <> '') and (Copy(mesmesprotocolo.AsString, 1, 6) <> '000000') then
    begin
      // Já autorizada (possui protocolo): apenas reimprime o documento existente, nunca gera nova chave.
      try
        VerificaPreExistencia('55');
      except
        on E: Exception do
    ...
    begin
      // Tem chave vinculada mas sem protocolo de autorização: estado inconsistente.
      // Bloqueia a geração de nova chave e orienta a consulta à SEFAZ.
      showmessage('100090 - Esta nota já possui uma chave vinculada (' + mesmeschavenfe.AsString +
        ') sem protocolo de autorização. Use "Consultar SEFAZ" para recuperar o status. ' +
        'A emissão foi bloqueada para evitar duplicidade fiscal.');
      exit;
    end;
  end;
  ...
  Begin
    ...
    begin
      ...
      begin
        try
          ...
          on E: Exception do
            SalvarLogErro(E.Message, E.StackTrace); // [#7] não engolir exceção silenciosamente
        end;
      end;
    end;
    (*
    ...
    *)
    ...
    // [#2] Reconciliação de totais ANTES de assinar/consumir número.
    // Confere o total informado (mesmestotal, calculado por outro módulo e gravado em Total.ICMSTot.vNF)
    // contra a soma dos componentes apurados item a item que vão no XML, pela fórmula oficial da NF-e 4.0:
    //   vNF = vProd - vDesc - vICMSDeson + vST + vFCPST + vFrete + vSeg + vOutro + vII + vIPI + vIPIDevol
    // (FCP normal já está embutido no ICMS; IBS/CBS são totalizados à parte e não compõem vNF.)
    // Divergência acima da tolerância bloqueia a emissão, evitando enviar valores inconsistentes à SEFAZ.
    vlNFCalculado := RoundTo(
        Total.ICMSTot.vProd
      - Total.ICMSTot.vDesc
      ...
      + Total.ICMSTot.vII
      + Total.ICMSTot.vIPI
      + Total.ICMSTot.vIPIDevol, -2);

    if Abs(vlNFCalculado - RoundTo(Total.ICMSTot.vNF, -2)) > 0.02 then
    begin
      showmessage('100091 - Diverg'#234'ncia no total da NF-e. ' +
        'Total informado (banco): R$ ' + formatfloat('#,##0.00', Total.ICMSTot.vNF) + '   /   ' +
        'Total calculado (itens + frete + outras - desconto + impostos): R$ ' + formatfloat('#,##0.00', vlNFCalculado) + '. ' +
        'Diferen'#231'a: R$ ' + formatfloat('#,##0.00', Abs(vlNFCalculado - Total.ICMSTot.vNF)) + '. ' +
        'Emiss'#227'o bloqueada para evitar envio de valores inconsistentes '#224' SEFAZ.');
      exit;
    end;



    // [#4] Pré-validação fiscal dos itens já montados (NFe.Det), antes de numerar/assinar.
    // Valida o FORMATO exigido pela SEFAZ (NCM com 8 dígitos, CFOP com 4 dígitos) sobre os
    // valores que realmente vão no XML — após todos os ajustes de CFOP/CST. Lista todas as
    // pendências numa única mensagem, evitando descobrir o erro só na rejeição da SEFAZ.
    vlErrosFiscais := TStringList.Create;
    try
      for i := 0 to ACBrNFeNotas.NotasFiscais.Items[0].NFe.Det.Count - 1 do
      begin
        with ACBrNFeNotas.NotasFiscais.Items[0].NFe.Det.Items[i].Prod do
        begin
    ...
    *)
    ...
    try
      ...
      // [#5] Persiste o ide.cNF (código numérico da NF-e) já na nota pendente, para que a
      // reemissão reaproveite o mesmo código (ver 'if mesmescodigonota.AsInteger<>0' acima) e
      // a chave permaneça recuperável mesmo sem o XML em disco.
      qconsulta.SQL.add('mescodigonota = ' + IntToStr(ide.cNF) + ', ');
      qconsulta.SQL.add('temcodigo = 4 ');
      qconsulta.SQL.add('WHERE meschave = ' + vpMesChave + ' and flacodigo=' + Acesso.Filial.ToString);
      qconsulta.ExecSQL;
    except
      on E: Exception do
        SalvarLogErro(E.Message, E.StackTrace); // [#7] registra falha ao gravar nota pendente
    end;
  ...
  Except
    ...
    Begin
      ...
      Begin
        ...
        // [#3] mesdatanfe usa sempre ide.dEmi (data de emissão), não a data de autorização (DhRecbto).
        consulta.SQL.add('mesdatanfe = ' + QuotedStr(ajustadata(DateToStr(ACBrNFeNotas.NotasFiscais.Items[0].NFe.ide.dEmi))) + ', ');
 ...
 { vlTentativasEnvio := 1;
  ...
  Begin
    ...
    qconsulta.SQL.Text := 'UPDATE mes SET ';
    qconsulta.SQL.add('mesregistro = ' + QuotedStr(ajustadata(DateToStr(vpDataAtual))) + ', ');
    // [#3] mesdatanfe usa sempre ide.dEmi (data de emissão do XML) como fonte única e imutável.
    qconsulta.SQL.add('mesdatanfe = ' + QuotedStr(ajustadata(DateToStr(ACBrNFeNotas.NotasFiscais.Items[0].NFe.ide.dEmi))) + ', ');
    qconsulta.SQL.add('mesnumero = ' + IntToStr(vNumeroNFe) + ', ');
    qconsulta.SQL.add('tdfcodigo = ' + QuotedStr('55') + ', ');
