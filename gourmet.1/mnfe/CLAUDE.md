...
#### Numbering is per-model — never cross sequences

NF-e (modelo 55) and NFC-e (modelo 65) have **separate** number sequences, stored in distinct columns of `cfgmnfe`: `cfgnumeronfe`/`cfgserienfe` for NF-e, `cfgnumeronfce`/`cfgserienfce` for NFC-e. The next number is drawn from a dedicated stored procedure per model: `CALL NumeroNFe(:flacodigo)` (NF-e) and `CALL NumeroNFCe` (NFC-e).

Both forms also **reuse** a number already persisted on the `mes` record (`mesnumero`) instead of drawing a new one — needed for retransmissions. But `mes.mesnumero` is shared storage across both models, so reuse must be guarded by the record's own model `mes.tdfcodigo` (`tdfNotaFiscalEletronica` = `'55'`, `tdfNotaFiscalConsumidorEletronica` = `'65'`, both from `uFuncoes`). **Only reuse the stored number when `mestdfcodigo` matches the model being emitted**; otherwise draw a fresh number from that model's sequence. Without this guard an NF-e could pick up a number generated by the NFC-e flow (and vice-versa). The guard lives at the "Identifica o Número" block in `ufnfe.pas` (`= tdfNotaFiscalEletronica`) and the mirrored block in `ufnfce.pas` (`= tdfNotaFiscalConsumidorEletronica`) — keep them symmetric.
